...
private:
    ...
    std::vector<rw::math::Vector3D<double>> singleTargOnTexture;
    std::vector<rw::math::Vector3D<double>> multipleTargOnTexture;
    std::vector<rw::math::Vector3D<double>> *currentTargOnTexture;
    std::vector<rw::math::Vector2D<int>> targetPixels;
    rw::math::Q velocity_limits;

    std::vector<rw::math::Vector3D<double>> getTargetsInCameraFrame(rw::math::Transform3D<double> cameraTtexture, std::vector<rw::math::Vector3D<double>> multipleTargOnTexture);
    ...
    rw::math::Q algorithm2();
    std::vector<rw::math::Vector2D<int>> cameraModel(std::vector<rw::math::Vector3D<double>> pointsInCameraFrame);
    void sim_dTs();
};
